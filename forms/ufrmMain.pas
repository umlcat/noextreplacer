...
begin
  AppPath := ParamStr(0);
  //@to-do: folder sep.
  ConfigFilename := ExtractFileDir(AppPath) + DirectorySeparator + 'noextreplacer.txt';
  ...
  begin
    ...
    begin
      if (
        ...
        (not FileRecord.IsParentFolder)) and
        (FileRecord.FileExt = '') then
      begin
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        if (uktstrings.SameText(Sourcename, 'readme')) then
        begin
          Destname := SysUtils.ChangeFileExt(Destname, '.txt');
        end
        else if (uktstrings.SameText(Sourcename, 'leeme')) then
        begin
          Destname := SysUtils.ChangeFileExt(Destname, '.txt');
        end
        else if (uktstrings.SameText(Sourcename, 'makefile')) then
        begin
          Destname := SysUtils.ChangeFileExt(Destname, '.mk');
        end
        else if (uktstrings.SameText(Sourcename, 'copying')) then
        begin
          Destname := SysUtils.ChangeFileExt(Destname, '.txt');
        end
        else
        begin
          Destname := SysUtils.ChangeFileExt(Destname, '.dat');
        end;
        ...
        begin
          uktfilesbyname.FileNameRename(FullSourcename, FullDestname);
